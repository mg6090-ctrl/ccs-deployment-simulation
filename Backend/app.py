...
def run_model():
    ...
    if "scenario1" in scenarios:
        ...
        s1_clusters = s1_params.get('clusters', project_data.CLUSTERS)
        s1_caps = s1_params.get('caps', project_data.CAPTURE)
        s1_caps_vol = s1_params.get('caps_vol', project_data.CAPTURE_VOLUMES)
        s1_trans = s1_params.get('trans', project_data.TRANSPORT)
        s1_trans_vol = s1_params.get('trans_vol', project_data.TRANSPORT_VOLUMES)
        s1_stor = s1_params.get('stor', project_data.STORAGE)
        s1_stor_vol = s1_params.get('stor_vol', project_data.STORAGE_VOLUMES)
    ...
    if "scenario2" in scenarios:
        ...
        s2_clusters = s2_params.get('clusters', project_data.CLUSTERS)
        s2_caps = s2_params.get('caps', project_data.CAPTURE)
        s2_caps_vol = s2_params.get('caps_vol', project_data.CAPTURE_VOLUMES)
        s2_trans = s2_params.get('trans', project_data.TRANSPORT)
        s2_trans_vol = s2_params.get('trans_vol', project_data.TRANSPORT_VOLUMES)
        s2_stor = s2_params.get('stor', project_data.STORAGE)
        s2_stor_vol = s2_params.get('stor_vol', project_data.STORAGE_VOLUMES)
